...
export default function AwardsPage() {
  ...
  const filteredAwards = activeFilter === "All" 
    ? awards 
    : awards.filter(award => 
        (award.category || "").toLowerCase().includes(activeFilter.toLowerCase())
      );
      ...
      <section className="relative flex flex-col justify-center min-h-[400px] md:min-h-[480px] pt-28 md:pt-36 pb-16 overflow-hidden bg-[#0a1835] border-b border-brand-gold/10">
        ...
        <div className="container-custom relative z-10 text-white">
          <div className="max-w-3xl animate-fade-up">
            <span className="text-[10px] font-black text-brand-gold uppercase tracking-[0.4em] mb-4 md:mb-6 block">Awards & Milestones</span>
            <h1 className="text-5xl md:text-8xl font-heading font-black text-white mb-6 md:mb-8 leading-[1.1]">
              Legacy of <span className="text-brand-gold italic">Impact.</span>
            </h1>
            <p className="text-white/90 text-lg md:text-xl font-light leading-relaxed max-w-2xl">
              Celebrating the prestigious honors, district recognitions, and historic milestones that define our club's journey toward excellence and service.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container-custom">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 mb-10 md:mb-16 animate-fade-up">
            ...
            <div className="flex items-center gap-2 md:gap-3 overflow-x-auto pb-2 md:pb-0 w-full hide-scrollbar px-1 -mx-1 md:px-0 md:mx-0">
              {["All", "District", "Zonal", "International", "Club", "Special Recognition"].map((tag) => (
                <button
                  key={tag}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-16 lg:gap-20">
            {loading ? (
              ...
              paginatedAwards.map((award: any, idx: number) => (
                <Link
                  key={idx}
                ...
                >
                  <div className="relative h-72 overflow-hidden">
                    ...
                    <div className="absolute top-6 left-6 flex flex-col gap-2 z-20">
                      ...
                      <div className="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest rounded-full shadow-sm backdrop-blur-md border bg-green-100/90 text-green-900 border-green-200">
                        Achieved
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-6 md:p-10 flex flex-col flex-1 bg-white">
                    <div className="text-brand-blue/40 text-[10px] font-black uppercase tracking-[0.2em] mb-4">{award.year}</div>
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="mt-16 md:mt-24 flex items-center justify-center gap-3">
              <button
                disabled={currentPage === 1}
                onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))}
                className="w-12 h-12 rounded-full border border-slate-200 bg-white text-brand-blue font-bold flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed hover:bg-brand-blue hover:text-white transition-colors"
              >
                &larr;
              </button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  ...
                  className={`w-12 h-12 rounded-full text-sm font-black transition-all ${currentPage === page
                    ? "bg-brand-blue text-white shadow-lg scale-110"
                    : "bg-white text-brand-blue border border-slate-200 hover:border-brand-blue"
                    }`}
                >
                  {page}
                </button>

...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    const staticRoutes = ['', '/about', '/projects', '/team', '/blogs', '/contact'].map((route) => ({
        url: `https://rcsb-website.pages.dev${route}`,
    ...
    try {
        ...
        const res = await fetch(`https://rcsb-website.pages.dev/api/projects`, { next: { revalidate: 3600 } });
        if (res.ok) {
            const data = await res.json();
            projectUrls = data.map((item: any) => ({
                url: `https://rcsb-website.pages.dev/${item.type === 'blog' ? 'blogs' : 'projects'}/${item.slug}`,

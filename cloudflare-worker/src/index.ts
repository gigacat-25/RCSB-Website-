...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/projects") {
        ...
        const SUPER_ADMIN = "rota.rcbs@gmail.com";
        ...
        if (workflowBinding) {
          try {
            await workflowBinding.create({
              id: `${insertedType}-${insertedId}-${Date.now()}`,
      ...
      if (matchProject) {
        ...
        if (request.method === "PUT" || request.method === "DELETE") {
          ...
          const isMasterAdmin = userEmail.toLowerCase() === "rota.rcbs@gmail.com";
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    const siteUrl = env.SITE_URL || "https://rotaractswarnabengaluru.in";
